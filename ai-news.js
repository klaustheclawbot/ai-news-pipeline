...
async function getAINews() {
  const [xSignals, rssSignals, webSignals] = await Promise.all([fetchXSignals(), fetchRssSignals(), fetchWebSignals()]);
  const combined = uniqueBy([...xSignals, ...rssSignals, ...webSignals], item => `${item.sourceType}:${item.lab || ''}:${item.title}`)
...
if (require.main === module) {
  getAINews().then(data => console.log(JSON.stringify(data, null, 2))).catch(err => {
    console.error(err);
    process.exit(1);
  });
}

module.exports = { getAINews, SOURCES };
